...
async function cmdWhoami() {
    ...
    if (result.agentCount !== undefined)
        out.agent_count = result.agentCount;
    if (result.budgetLimitSats !== undefined)
        out.budget_limit_sats = result.budgetLimitSats;
    if (result.operatorId !== undefined)
        out.operator_id = result.operatorId;
    return out;
}
...
async function cmdPay(positional, flags) {
    const invoice = positional[0];
    if (!invoice)
        error('Usage: lw pay <bolt11_invoice>');
    const client = getClient();
    const maxFee = flags['max-fee'] ? parseInt(flags['max-fee'], 10) : undefined;
...
async function cmdPayApi(positional, flags) {
    const url = positional[0];
    if (!url)
        error('Usage: lw pay-api <url> [--method GET] [--body "{}"] [--max-sats 1000]');
    const client = getClient();
    const method = flags.method || 'GET';
    const body = flags.body;
    const maxSats = flags['max-sats'] ? parseInt(flags['max-sats'], 10) : 1000;
    ...
    if (result.paymentProtocol)
        out.payment_protocol = result.paymentProtocol;
    if (result.usdcAmount)
        out.usdc_amount = result.usdcAmount;
    return out;
}
async function cmdCreateAgent(positional, flags) {
    const name = positional[0] || flags.name;
    if (!name)
        error('Usage: lw create-agent <name> [--budget <sats>]');
    const client = getClient();
...
async function cmdFundAgent(positional) {
    const agentId = positional[0];
    const amount = positional[1];
    if (!agentId || !amount)
        error('Usage: lw fund-agent <agent_id> <amount_sats>');
    const client = getClient();
...
async function cmdDeposit(positional) {
    const amount = positional[0];
    if (!amount)
        error('Usage: lw deposit <amount_sats>');
    const client = getClient();
...
async function cmdWithdraw(positional) {
    const invoice = positional[0];
    if (!invoice)
        error('Usage: lw withdraw <bolt11_invoice>');
    const client = getClient();
...
async function cmdWithdrawLink(positional) {
...
async function cmdDecode(positional) {
    const invoice = positional[0];
    if (!invoice)
        error('Usage: lw decode <bolt11_invoice>');
    const client = getClient();
...
function cmdUseKey(positional, flags) {
    ...
    if (!savedTo) {
        ...
        error(`Could not save credentials to ${(0, credentials_js_1.credentialsPath)()} (persistence disabled or directory not writable). Set the LIGHTNING_WALLET_API_KEY environment variable to the key you passed instead.`);
    }
