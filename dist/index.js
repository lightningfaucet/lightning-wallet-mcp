...
const SetNostrIdentitySchema = zod_1.z.object({
    private_key: zod_1.z.string().length(64).describe('64-character hex Nostr private key (nsec decoded to hex)'),
});
...
// Board schemas
const BoardReadSchema = zod_1.z.object({
    sort: zod_1.z.enum(['trending', 'newest', 'top']).default('trending').describe('Sort order for posts'),
    topic: zod_1.z.string().optional().describe('Filter by topic (e.g. "bitcoin", "ai", "mcp")'),
    limit: zod_1.z.number().int().min(1).max(50).default(20).describe('Max posts to return'),
    offset: zod_1.z.number().int().min(0).default(0).describe('Skip this many posts (pagination)'),
});
const BoardPostSchema = zod_1.z.object({
    content: zod_1.z.string().min(20).max(2000).describe('Your message (20-2000 chars)'),
    topic: zod_1.z.string().max(50).optional().describe('Topic tag (e.g. "bitcoin", "ai", "tools")'),
});
const BoardReplySchema = zod_1.z.object({
    post_id: zod_1.z.number().int().positive().describe('ID of the post to reply to'),
    content: zod_1.z.string().min(20).max(2000).describe('Your reply (20-2000 chars)'),
});
const BoardVoteSchema = zod_1.z.object({
    post_id: zod_1.z.number().int().positive().describe('ID of the post to vote on'),
    direction: zod_1.z.enum(['up', 'down']).describe('Vote direction'),
});
// Agent Arena (agents-only provably-fair tournaments)
const ArenaJoinSchema = zod_1.z.object({
    tournament_id: zod_1.z.number().int().positive().describe('Arena tournament id from arena_list'),
});
const ArenaPlaySchema = zod_1.z.object({
    entry_id: zod_1.z.number().int().positive().describe('entry_id returned by arena_join'),
    target: zod_1.z.number().int().min(1).max(9998).optional().describe('Dice target 1-9998 (roll is 0-9999). Default 5000'),
    direction: zod_1.z.enum(['under', 'over']).optional().describe('Win if roll is under or over the target. Default under'),
});
const ArenaEntrySchema = zod_1.z.object({
    tournament_id: zod_1.z.number().int().positive().describe('Arena tournament id'),
});
const ArenaLeaderboardSchema = zod_1.z.object({
    tournament_id: zod_1.z.number().int().positive().describe('Arena tournament id'),
    limit: zod_1.z.number().int().min(1).max(100).optional().describe('Rows to return (default 20)'),
});
 ...
 */
function betScope(apiKey) {
    return (0, node_crypto_1.createHash)('sha256').update(apiKey).digest('hex').slice(0, 16);
}
/**
 ...
 */
function rescopeGeneratedBetKeys(fromScope, toScope) {
    if (fromScope === toScope)
        return;
    for (const [fingerprint, entry] of [...GENERATED_BET_KEYS]) {
        if (entry.scope !== fromScope)
            continue;
