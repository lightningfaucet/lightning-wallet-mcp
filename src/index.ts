...
class SessionState {
  ...
  constructor() {
    ...
    const stored = activeStoredKey(loadCredentials());
    if (stored) {
      this.client = new LightningFaucetClient(stored.api_key);
      this.keySource = 'file';
    }
  }
...
const GetTransactionsSchema = z.object({
  limit: z.number().min(1).max(100).default(50).describe('Max transactions to return'),
...
const RegisterWebhookSchema = z.object({
  ...
  events: z.array(z.enum(['invoice_paid', 'payment_completed', 'payment_failed', 'withdrawal_completed', 'balance_changed', 'balance_low', 'budget_warning', 'bet_placed', 'bet_settled', 'test']))
...
const PredictionMarketsSchema = z.object({
  status: z.enum(['open', 'upcoming', 'closed', 'resolved', 'cancelled', 'all']).optional().describe('Market status filter. Default open'),
  market_type: z.enum(['sports', 'btc_price']).optional().describe('sports (fixed-odds books and series) or btc_price (parimutuel)'),
  category: z.string().min(1).max(40).optional().describe('League or category key, e.g. nfl, nba, nhl, mlb, ncaaf, mma, epl, ucl, tennis, btc_daily'),
...
const PredictionMarketSchema = z.object({
  market_id: z.number().int().positive().describe('Market id from prediction_markets'),
});
const PredictionPlaceBetSchema = z.object({
  market_id: z.number().int().positive().describe('Market id from prediction_markets'),
  position: z.enum(['yes', 'no']).describe('Side to back'),
  ...
  (p) => (p.expected_odds_pct === undefined) === (p.expected_line_version === undefined),
  { message: 'expected_odds_pct and expected_line_version must be passed together (both or neither)', path: ['expected_line_version'] },
);
...
function predictionNextHint(error: string): string {
  switch (error) {
    case 'odds_changed':
...
const PredictionMyBetsSchema = z.object({
  ...
  agent_id: z.number().int().positive().optional().describe('Operator key only: limit to one agent'),
  limit: z.number().int().min(1).max(100).optional().describe('Rows to return (default 50)'),
  offset: z.number().int().min(0).optional().describe('Pagination offset'),
});
  ...
  try {
    switch (name) {
      ...
      case 'pay_l402_api': {
        ...
        const backendPayment = ((result.rawResponse as unknown as Record<string, unknown>)?.payment ?? null) as Record<string, unknown> | null;
        ...
        const l402Raw = result.rawResponse as unknown as Record<string, unknown>;
        for (const k of ['via_agent_id', 'default_agent_created', 'returned_to_operator_sats'] as const) {
          if (l402Raw && l402Raw[k] !== undefined) responseData[k] = l402Raw[k];
        }
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                ...(((result.rawResponse as unknown as Record<string, unknown>)?.default_agent_created !== undefined) ? { default_agent_created: true } : {}),
                ...(((result.rawResponse as unknown as Record<string, unknown>)?.returned_to_operator_sats !== undefined) ? { returned_to_operator_sats: (result.rawResponse as unknown as Record<string, unknown>).returned_to_operator_sats } : {}),
              }, null, 2),
            },
          ],
        };
      ...
      case 'register_operator': {
        ...
        const savedTo = saveOperatorKey(result.apiKey, {
      ...
      case 'forget_credentials': {
        const removed = forgetCredentials();
      ...
      case 'set_operator_key': {
        ...
        session.keySource = 'file';
        const opSavedTo = saveOperatorKey(parsed.api_key);
        return {
          content: [
            {
              type: 'text',
      ...
      case 'recover_account': {
        ...
        session.setClient(new LightningFaucetClient(result.apiKey));
        session.keySource = 'file';
        const recSavedTo = saveOperatorKey(result.apiKey, { id: result.operatorId, recovery_code: parsed.recovery_code });
        return {
          content: [
            {
              type: 'text',
      ...
      case 'rotate_api_key': {
        ...
        const rotSavedTo = parsed.agent_id
          ? saveAgentKey(result.apiKey, { id: parsed.agent_id }, true)
          : saveOperatorKey(result.apiKey, {}, { rotatedFrom: previousKey });
        return {
          content: [
            {
              type: 'text',
      ...
      case 'prediction_markets': {
        ...
        const result = c ? await c.predictionMarkets(filters) : await getPublicAction('prediction_markets', filters);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case 'prediction_market': {
        ...
        const c = session.getClient();
        const result = c ? await c.predictionMarket(parsed.market_id) : await getPublicAction('prediction_market', { market_id: parsed.market_id });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case 'prediction_place_bet': {
        ...
        try {
          ...
          if (e instanceof ApiError && PREDICTION_REFUSALS.has(String(e.response.error ?? ''))) {
    ...
    if (error instanceof ApiError) {
      ...
      for (const k of ['hint', 'cooldown_until', 'seconds_remaining', 'required', 'available', 'balance_sats', 'shortfall_sats', 'min_amount_sats'] as const) {
